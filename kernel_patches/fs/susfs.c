...
static DEFINE_MUTEX(susfs_mutex_lock_file_spoof);
static DEFINE_HASHTABLE(FILE_SPOOF_HLIST, 10);
DEFINE_STATIC_SRCU(susfs_srcu_file_spoof);

static bool susfs_file_spoof_uid_allowed(int uid_scheme) {
	switch (uid_scheme) {
	case UID_NON_APP_PROC:
...
void susfs_add_file_spoof(void __user **user_info) {
	struct st_susfs_file_spoof *info = NULL;
	struct st_susfs_file_spoof_hlist *new_entry = NULL;
	struct st_susfs_file_spoof_hlist *old_entry = NULL;
	struct st_susfs_file_spoof_hlist *tmp_entry = NULL;
	struct hlist_node *tmp_hlist_node;
	...
	info = kzalloc(sizeof(struct st_susfs_file_spoof), GFP_KERNEL);
	if (!info) {
		err = -ENOMEM;
		if (copy_to_user(&((struct st_susfs_file_spoof __user*)*user_info)->err, &err, sizeof(err)))
			err = -EFAULT;
		SUSFS_LOGI("CMD_SUSFS_ADD_FILE_SPOOF -> ret: %d\n", err);
		return;
	}

	if (copy_from_user(info, (struct st_susfs_file_spoof __user*)*user_info, sizeof(struct st_susfs_file_spoof))) {
		info->err = -EFAULT;
		goto out_copy_to_user;
	}
	...
	if (info->spoofed_size > SUSFS_MAX_SPOOF_FILE_SIZE) {
		info->err = -EINVAL;
		SUSFS_LOGE("spoofed_size is too large: %u\n", info->spoofed_size);
		goto out_copy_to_user;
	}
	...
	new_entry = kzalloc(sizeof(struct st_susfs_file_spoof_hlist), GFP_KERNEL);
	if (!new_entry) {
		info->err = -ENOMEM;
		goto out_path_put_target_path;
	}

	new_content = kzalloc(SUSFS_MAX_SPOOF_FILE_SIZE, GFP_KERNEL);
	if (!new_content) {
		info->err = -ENOMEM;
		goto out_free_new_entry;
	}

	if (info->spoofed_size)
		memcpy(new_content, info->spoofed_content, info->spoofed_size);

	new_entry->target_ino = target_inode->i_ino;
	new_entry->target_dev = target_inode->i_sb->s_dev;
	new_entry->spoofed_content = new_content;
	new_entry->spoofed_size = info->spoofed_size;
	new_entry->uid_scheme = info->uid_scheme;
	strscpy(new_entry->target_pathname, info->target_pathname, SUSFS_MAX_LEN_PATHNAME - 1);

	mutex_lock(&susfs_mutex_lock_file_spoof);
	hash_for_each_possible_safe(FILE_SPOOF_HLIST, tmp_entry, tmp_hlist_node, node, target_inode->i_ino) {
		if (tmp_entry->target_dev == target_inode->i_sb->s_dev &&
			!strcmp(tmp_entry->target_pathname, info->target_pathname)) {
			old_entry = tmp_entry;
	...
	hash_add_rcu(FILE_SPOOF_HLIST, &new_entry->node, new_entry->target_ino);
	set_bit(AS_FLAGS_FILE_SPOOF, &target_inode->i_mapping->flags);
	mutex_unlock(&susfs_mutex_lock_file_spoof);

	if (old_entry_found) {
		synchronize_srcu(&susfs_srcu_file_spoof);
		kfree(old_entry->spoofed_content);
		kfree(old_entry);
	}

	SUSFS_LOGI("target_pathname: '%s', target_i_ino: '%lu', target_s_dev: '%lu', spoofed_size: '%u', uid_scheme: '%d', is successfully added to FILE_SPOOF_HLIST\n",
			new_entry->target_pathname, new_entry->target_ino, new_entry->target_dev, new_entry->spoofed_size, new_entry->uid_scheme);
	info->err = 0;
	goto out_path_put_target_path;
...
out_path_put_target_path:
	path_put(&target_path);
out_copy_to_user:
	if (copy_to_user(&((struct st_susfs_file_spoof __user*)*user_info)->err, &info->err, sizeof(info->err))) {
		info->err = -EFAULT;
	}
	SUSFS_LOGI("CMD_SUSFS_ADD_FILE_SPOOF -> ret: %d\n", info->err);
	kfree(info);
...
static bool susfs_file_spoof_path_matches(struct file *file, struct st_susfs_file_spoof_hlist *entry) {
	char *pathname = NULL;
	char *dpath = NULL;
	bool matched = false;
	...
	dpath = d_path(&file->f_path, pathname, SUSFS_MAX_LEN_PATHNAME);
	if (!IS_ERR(dpath) && !strcmp(dpath, entry->target_pathname))
		matched = true;
...
static struct st_susfs_file_spoof_hlist *susfs_file_spoof_get_entry(struct file *file) {
	struct inode *inode = file_inode(file);
	struct st_susfs_file_spoof_hlist *entry = NULL;

	if (!SUSFS_IS_INODE_FILE_SPOOF_WITHOUT_UID_CHECK(inode))
		return NULL;

	hash_for_each_possible_rcu(FILE_SPOOF_HLIST, entry, node, inode->i_ino) {
		if (entry->target_dev == inode->i_sb->s_dev &&
			susfs_file_spoof_uid_allowed(entry->uid_scheme) &&
			susfs_file_spoof_path_matches(file, entry))
			return entry;
	}
...
ssize_t susfs_spoof_file_read(struct file *file, char __user *buf, size_t count, loff_t *pos) {
	struct st_susfs_file_spoof_hlist *entry = NULL;
	...
	srcu_idx = srcu_read_lock(&susfs_srcu_file_spoof);
	entry = susfs_file_spoof_get_entry(file);
	if (!entry)
		goto out_srcu_read_unlock;
	...
	if (ppos >= entry->spoofed_size) {
		ret = 0;
		goto out_srcu_read_unlock;
	}

	offset = (size_t)ppos;
	readable_size = entry->spoofed_size - offset;
	copy_size = min(count, readable_size);

	if (copy_to_user(buf, entry->spoofed_content + offset, copy_size)) {
		ret = -EFAULT;
		goto out_srcu_read_unlock;
	}
	...
	if (pos)
		*pos = ppos + copy_size;
		...
		file->f_pos = ppos + copy_size;

	ret = copy_size;
	SUSFS_LOGI("spoof read target '%s', copied: %zd/%u, uid_scheme: %d\n",
			entry->target_pathname, ret, entry->spoofed_size, entry->uid_scheme);

out_srcu_read_unlock:
	srcu_read_unlock(&susfs_srcu_file_spoof, srcu_idx);
	return ret;
}

ssize_t susfs_spoof_file_read_iter(struct file *file, struct iov_iter *to, loff_t *pos) {
	struct st_susfs_file_spoof_hlist *entry = NULL;
	loff_t ppos = pos ? *pos : file->f_pos;
	...
	readable_size = entry->spoofed_size - offset;
	copy_size = min(count, readable_size);
	copied = copy_to_iter(entry->spoofed_content + offset, copy_size, to);
	if (copied != copy_size) {
		ret = -EFAULT;
		goto out_srcu_read_unlock;
	}
	...
	if (pos)
		*pos = ppos + copied;
		...
		file->f_pos = ppos + copied;

	ret = copied;
	SUSFS_LOGI("spoof iter read target '%s', copied: %zd/%u, uid_scheme: %d\n",
			entry->target_pathname, ret, entry->spoofed_size, entry->uid_scheme);
...
static DEFINE_MUTEX(susfs_mutex_lock_sus_map);
static DEFINE_HASHTABLE(SUS_MAP_HLIST, 10);
DEFINE_STATIC_SRCU(susfs_srcu_sus_map);

static bool susfs_sus_map_uid_allowed(int uid_scheme) {
	switch (uid_scheme) {
	case UID_NON_APP_PROC:
		return current_uid().val % 100000 < 10000;
	case UID_ROOT_PROC_EXCEPT_SU_PROC:
...
bool susfs_sus_map_inode_allowed(struct inode *inode) {
	struct st_susfs_sus_map_hlist *entry = NULL;
	bool found = false;
	bool allowed = false;
	int srcu_idx;
	...
	srcu_idx = srcu_read_lock(&susfs_srcu_sus_map);
	hash_for_each_possible_rcu(SUS_MAP_HLIST, entry, node, inode->i_ino) {
		if (entry->target_dev != inode->i_sb->s_dev)
			continue;

		found = true;
		if (susfs_sus_map_uid_allowed(entry->uid_scheme)) {
			allowed = true;
			break;
		}
	}
	srcu_read_unlock(&susfs_srcu_sus_map, srcu_idx);

	/*
	 * Backward compatibility for older in-memory flags that were created
	 * before the uid-aware hlist existed. New add_sus_map() calls add an
	 * UID_UMOUNTED_APP_PROC entry, so this path is normally unused.
	 */
	if (!found)
		return susfs_is_current_proc_umounted_app();
...
static int susfs_add_sus_map_internal(const char *target_pathname, int uid_scheme) {
	struct st_susfs_sus_map_hlist *new_entry = NULL;
	struct st_susfs_sus_map_hlist *tmp_entry = NULL;
	struct hlist_node *tmp_hlist_node;
	struct path path;
	struct inode *inode = NULL;
	int err;

	if (!target_pathname || *target_pathname == '\0')
		return -EINVAL;

	if (uid_scheme < UID_NON_APP_PROC || uid_scheme > UID_UMOUNTED_PROC)
		return -EINVAL;

	err = kern_path(target_pathname, LOOKUP_FOLLOW, &path);
	if (err) {
		SUSFS_LOGE("failed opening file '%s'\n", target_pathname);
		return err;
	}
	...
	mutex_lock(&susfs_mutex_lock_sus_map);
	hash_for_each_possible_safe(SUS_MAP_HLIST, tmp_entry, tmp_hlist_node, node, inode->i_ino) {
		if (tmp_entry->target_dev == inode->i_sb->s_dev &&
			tmp_entry->uid_scheme == uid_scheme) {
			set_bit(AS_FLAGS_SUS_MAP, &inode->i_mapping->flags);
			mutex_unlock(&susfs_mutex_lock_sus_map);
			SUSFS_LOGI("pathname: '%s', sus_map uid_scheme: '%d' already exists\n",
					target_pathname, uid_scheme);
			err = 0;
			goto out_path_put_path;
		}
	}
	...
	new_entry->target_dev = inode->i_sb->s_dev;
	new_entry->uid_scheme = uid_scheme;
	hash_add_rcu(SUS_MAP_HLIST, &new_entry->node, new_entry->target_ino);
	set_bit(AS_FLAGS_SUS_MAP, &inode->i_mapping->flags);
	mutex_unlock(&susfs_mutex_lock_sus_map);

	SUSFS_LOGI("pathname: '%s', ino: '%lu', dev: '%lu', uid_scheme: '%d', is flagged as AS_FLAGS_SUS_MAP\n",
			target_pathname, inode->i_ino, inode->i_sb->s_dev, uid_scheme);
	err = 0;
...
void susfs_add_sus_map(void __user **user_info) {
	...
	info.err = susfs_add_sus_map_internal(info.target_pathname, UID_UMOUNTED_APP_PROC);
...
void susfs_add_sus_map_uid(void __user **user_info) {
	struct st_susfs_sus_map_uid info = {0};

	if (copy_from_user(&info, (struct st_susfs_sus_map_uid __user*)*user_info, sizeof(info))) {
		info.err = -EFAULT;
		goto out_copy_to_user;
	}

	info.err = susfs_add_sus_map_internal(info.target_pathname, info.uid_scheme);

out_copy_to_user:
	if (copy_to_user(&((struct st_susfs_sus_map_uid __user*)*user_info)->err, &info.err, sizeof(info.err))) {
		info.err = -EFAULT;
	}
	SUSFS_LOGI("CMD_SUSFS_ADD_SUS_MAP_UID -> ret: %d\n", info.err);
}
...
#endif
#ifdef CONFIG_KSU_SUSFS_SPOOF_FILE
	info->err = copy_config_to_buf("CONFIG_KSU_SUSFS_SPOOF_FILE\n", buf_ptr, &copied_size, SUSFS_ENABLED_FEATURES_SIZE);
	if (info->err) goto out_copy_to_user;
	buf_ptr = info->enabled_features + copied_size;
#endif
#ifdef CONFIG_KSU_SUSFS_SUS_MAP
