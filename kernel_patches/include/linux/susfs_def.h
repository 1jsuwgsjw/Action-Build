...
#define SUSFS_MAX_SPOOF_FILE_SIZE 16384 // enough for /proc/cpuinfo and small procfs/sysfs identity nodes
...
#define SUSFS_IS_INODE_SUS_MAP(inode) \
		...
		susfs_sus_map_inode_allowed(inode)
...
#define SUSFS_IS_INODE_FILE_SPOOF_WITHOUT_UID_CHECK(inode) \
		inode && inode->i_mapping && \
		unlikely(test_bit(AS_FLAGS_FILE_SPOOF, &inode->i_mapping->flags))

#define SUSFS_IS_INODE_FILE_SPOOF(inode) \
		inode && inode->i_mapping && \
		unlikely(test_bit(AS_FLAGS_FILE_SPOOF, &inode->i_mapping->flags)) && \
		susfs_is_current_proc_umounted_app()
